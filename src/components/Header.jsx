...
const Header = () => {
  ...
  return (
    <div className="flex sticky top-0 w-full z-20 justify-between items-center px-[2rem] md:px-[3rem] lg:px-[5rem] h-[70px] font-nunito  bg-white">
      ...
      <li onClick={handleToggleDropdown} onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave} className={`relative text-black hover:text-brown font-medium transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 userIconContainer`}>
              <Link to='/'>
              ...
              {isDropdownVisible && (
                ...
                <Link to='signup' className="py-[6px] hover:bg-slate-200  ">Sign Up</Link>
                <Link to="login" className="py-[6px] hover:bg-slate-200 ">Login</Link>
              </div>
              )}
            </li>
        {navLinks2.map((link) => (
          ...
          {/* {link.text === ""} */}
            ...
            <p className="absolute top-[-5px] left-2 text-white rounded-full bg-red-600 px-1  text-[11px] "> {link.number} </p>
          </li>
        ))}
      </ul>
