...
const addToWishlistSlice = createSlice({
    ...
    reducers: {
        ...
        removeFromWishlist: (state,actions) => {
            ...
            const productAvailable = state.wishlistGroup.some((product)=> product.id === actions.payload.id); //check if id is already available in the array
            if(productAvailable){
                state.wishlistGroup = state.wishlistGroup.filter((product)=> product.id !== actions.payload.id); //if true, filter and return all id's that are not equal to it
            }
        },
    }
})
...
export const wishlistGroupSelector = (state) => state.addToWishlist.wishlistGroup;