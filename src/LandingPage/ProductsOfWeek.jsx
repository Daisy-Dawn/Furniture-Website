          ...
          {productsOfWeek.map((product, index) => (
            <div key={product.id} className='justify-self-center'>
              <DelayedSurface key={product.id} index={index}>
                <ProductsCard link={`/shop/${product.id}`} product={product} id={product.id} image={`https://freefurnitura.000webhostapp.com/reactApiPhp/images/${product.image}`} price={product.price} description={product.name} />
              </DelayedSurface>
            </div>
        ))}
        </div>
