  ...
  return (
    <ScrollSurface>
      ...
      {highlighProducts.map(product => (
        <HighlightComp id={product.id} key={product.id} image={`https://freefurnitura.000webhostapp.com/reactApiPhp/images/${product.image}`} heading={product.name} paragraph={product.description} button="Shop Now" />
      ))}
      </div>
    </ScrollSurface>
  )
