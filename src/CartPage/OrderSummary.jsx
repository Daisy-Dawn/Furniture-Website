...
import { cartTotalAmountSelector, cartListGroupSelector, getTotals, getSubTotal } from "../features/addToCartSlice";
...
const OrderSummary = () => {
  ...
  return (
    <section className="justify-self-center md:col-span-2 lg:col-span-1 w-full max-w-[38.6875rem] h-fit rounded-[0.625rem] py-8 px-4 border border-solid border-steam">
        <div>
            <h2 className="text-xl text-lead font-bold w-full max-w-[23.375rem] mb-5 md:mb-6">Order Summary</h2>
            <h3 className="text-lead text-base md:text-lg font-semi-bold flex justify-between">Price <span className="text-base font-normal">${cartTotalAmount}</span></h3>
            ...
            <h3 className="text-lead text-base md:text-lg font-semi-bold flex justify-between">Discount <span className={`text-base ${couponDiscount ? "text-red-700" : "text-lead"}  font-normal`}>${couponDiscount}</span></h3>
            <div className="flex justify-between items-center my-3 gap-2 ">
              <div className="flex border border-solid border-steam p-2 gap-2 rounded-[0.625rem]">
                ...
                <input onChange={(e)=> setCouponCode(e.target.value)} className="peer outline-none text-sm sm:text-base text-lead w-full max-w-[10.9375rem]" type="text" placeholder="Enter coupon code" />
              </div>
              <button onClick={()=> handleCheckCoupon(couponCode)} disabled={couponCode ? false : true} className="disabled:cursor-not-allowed disabled:text-steam cursor-pointer">Apply</button>
            </div>
            <hr/>
            <h3 className="text-lead text-base md:text-lg font-semi-bold flex justify-between my-3">TOTAL <span className="text-base font-normal">${orderSummaryTotal}</span></h3>
            {/* <h3 className="text-lead text-base md:text-lg font-semi-bold flex justify-between">Estimated Delivery by <span className="text-base font-normal">02 July, 2023</span></h3> */}
            <button 
